...
def create_app(args):
    ...
    try:
        rag = LightRAG(
            ...
            document_processing_batch_size=args.document_processing_batch_size,
    ...
    # Expose core instances for reuse in non-API scripts
    app.state.rag = rag
    app.state.doc_manager = doc_manager
    app.state.config_args = args

    # Add routes (ingestion and graph only)
    app.include_router(
        create_document_routes(
            rag,
            doc_manager,
