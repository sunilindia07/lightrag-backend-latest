...
def create_graph_routes(rag, api_key: Optional[str] = None):
    ...
    @router.get("/graph/data", dependencies=[Depends(combined_auth)])
    async def get_graph_data(
        ...
        Args:
            query: Entity or query to center the graph around
        ...
        """
        try:
            # If no query provided, get all entities up to max_nodes
            if not query or query.strip() == "" or query.strip() == "*":
                ...
                entities_to_process = list(all_entities.keys())[:max_nodes]
                is_truncated = len(all_entities) > max_nodes
                ...
                # Build edges
                all_relations = await rag.chunk_entity_relation_graph.get_all_edges()
                for edge_data in all_relations:
                    source = edge_data.get("src_id")
                    target = edge_data.get("tgt_id")
                    
                    # Only include edges where both nodes are in our node set
                    if source in entities_to_process and target in entities_to_process:
                        edge_key = f"{source}-{target}"
                        reverse_key = f"{target}-{source}"
                        
                        if edge_key not in processed_edges and reverse_key not in processed_edges:
                            edges.append({
                                "id": edge_key,
                                "source": source,
                                "target": target,
                ...
                # Query-specific graph data
                # Use the existing query mechanism to get related entities
                ...
                # Get context data which includes entities and relationships
                result = await rag.aquery(query, param=query_param)
                
                if not result or not hasattr(result, 'raw_data'):
                    return {
                        "nodes": [],
                        "edges": [],
                        "is_truncated": False
                ...
                if raw_data and raw_data.get("entities"):
                    for entity in raw_data["entities"]:
                        nodes.append({
                            "id": entity.get("entity_name", entity.get("id", "")),
                            "labels": [entity.get("entity_name", entity.get("id", ""))],
                            "properties": {
                                "entity_id": entity.get("entity_name", entity.get("id", "")),
                ...
                if raw_data and raw_data.get("relationships"):
                    for relation in raw_data["relationships"]:
